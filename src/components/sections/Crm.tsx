  ...
  return (
    <>
      ...
      >
        ...
        <div className="relative z-10 mx-auto grid w-full max-w-7xl items-center gap-12 px-6 lg:grid-cols-[1.1fr_0.9fr] lg:gap-14 lg:px-8">
          <motion.div initial={{ opacity: 1 }} animate={{ opacity: 1 }}>
            <div className="inline-flex items-center gap-3 border border-indigo-400/25 bg-indigo-500/5 px-4 py-2 font-mono text-xs uppercase tracking-[0.2em] text-indigo-300">
              <span className="h-px w-6 bg-indigo-400/70" />
              Command Center
            </div>
            ...
            <h1 className="mt-6 text-4xl font-extrabold leading-[1.08] tracking-tight text-text sm:text-5xl lg:text-6xl">
              One CRM.
              <br />
              <span className="bg-gradient-to-r from-indigo-300 via-violet to-fuchsia-300 bg-clip-text text-transparent">
          ...
          <motion.div
            ...
            className="relative"
          >
            <div className="absolute -inset-4 rounded-3xl bg-gradient-to-br from-indigo-500/25 via-transparent to-violet/15 blur-2xl" />

            <div className="relative overflow-hidden rounded-2xl border border-indigo-400/25 bg-panel/70 shadow-2xl shadow-black/40 backdrop-blur-xl">
              <div className="flex items-center justify-between border-b border-line px-5 py-4">
                <div>
                  <div className="text-sm font-semibold text-text">
                    Pipeline snapshot
                  </div>
                  <div className="mt-0.5 font-mono text-[11px] uppercase tracking-wider text-indigo-300">
                    Live · All channels
                  </div>
                </div>
              ...
              <div className="grid grid-cols-3 gap-px bg-line">
                {liveStats.map((s) => (
                  <div key={s.label} className="bg-bg/80 px-2 py-3 text-center sm:px-4 sm:py-4">
                    <div className="text-base font-bold text-indigo-300 sm:text-lg md:text-xl">
                      {s.value}
                    </div>
              ...
              <div className="relative aspect-[16/10] bg-bg-alt">
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-bg via-bg/50 to-transparent p-3 sm:p-4">
                  <div className="flex flex-col gap-1 font-mono text-[10px] uppercase tracking-wider text-text-dimmer sm:flex-row sm:items-center sm:justify-between sm:text-xs">
                    <span className="text-indigo-300">Deal board preview</span>
          ...
          <Stagger className="space-y-3">
            {features.map((f) => (
              <MotionItem key={f.num}>
                <div className="group flex gap-4 rounded-2xl border border-line bg-panel/40 p-5 transition-all hover:border-indigo-400/35 hover:bg-indigo-500/5">
        ...
        <FadeUp>
          <div className="mb-12 flex flex-wrap items-center justify-center gap-3">
            {platforms.map((name) => (
              <span
                key={name}
                className="rounded-full border border-indigo-400/20 bg-indigo-500/5 px-5 py-2 text-sm font-medium text-text-dim transition-colors hover:border-indigo-400/40 hover:text-indigo-200"
              >
                {name}
              </span>
            ))}
        ...
        <Stagger className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {syncPoints.map((item, i) => (
            <MotionItem key={item.title}>
              <Card className="h-full border-line bg-panel/40 transition-all hover:border-indigo-400/30 hover:bg-panel/70">
      ...
      <Section border alt id="crm-pipeline">
        ...
        <FadeUp>
          <div className="relative mx-auto max-w-5xl overflow-hidden rounded-3xl border border-indigo-400/20 bg-gradient-to-br from-panel/80 via-panel/40 to-indigo-500/5 p-8 shadow-[0_0_80px_rgba(99,102,241,0.08)] md:p-10">
            <div className="absolute left-[12%] right-[12%] top-[3.75rem] hidden h-px bg-gradient-to-r from-transparent via-indigo-400/40 to-transparent md:block" />
            <ol className="grid grid-cols-1 gap-8 sm:grid-cols-2 md:grid-cols-4 md:gap-4">
              {pipeline.map((step, i) => (
                <li key={step.stage} className="relative text-center">
                  <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full border border-indigo-400/30 bg-bg font-mono text-sm font-semibold text-indigo-300 shadow-[0_0_40px_rgba(99,102,241,0.2)]">
      ...
      <section className="relative overflow-hidden border-t border-line px-6 py-24 md:py-28">
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-indigo-500/10 via-transparent to-violet/5" />
        <FadeUp className="relative mx-auto max-w-3xl text-center">
          ...
          <div className="mt-9 flex flex-wrap items-center justify-center gap-4">
            ...
            <Link
              href="/services"
              className="inline-flex items-center rounded-full border border-line bg-panel/60 px-7 py-3.5 text-sm font-semibold text-text no-underline backdrop-blur-sm transition-all hover:border-indigo-400/40"
            >
              Explore Services
            </Link>
          </div>
